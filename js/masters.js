...
// Function to fetch ONLY Master's scholarships
const getMastersScholarships = async () => {
  try {
    const { data: scholarships, error } = await supabase
      .from('scholarships')
      .select('*')
      .eq('category', 'master') // Filter for master only
    ...
    console.error("Error getting Master's scholarships: ", error);
    return [];
  }
};
...
function displayScholarships(scholarships) {
  ...
  if (scholarships.length === 0) {
    container.innerHTML = '<p class="text-center w-100">No Master\'s scholarships found. Check back later!</p>';
    return;
  }
...
function highlightCurrentPage() {
  const currentPage = 'master.html'; // This page
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const scholarships = await getMastersScholarships(); // Fetch data
