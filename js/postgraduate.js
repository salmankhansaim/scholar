...
// Function to fetch ONLY Post Graduate scholarships
const getPostGraduateScholarships = async () => {
  try {
    const { data: scholarships, error } = await supabase
      .from('scholarships')
      .select('*')
      .eq('category', 'postgraduate') // Filter for postgraduate only
    ...
    console.error("Error getting Post Graduate scholarships: ", error);
    return [];
  }
};
...
function displayScholarships(scholarships) {
  ...
  if (scholarships.length === 0) {
    container.innerHTML = '<p class="text-center w-100">No Post Graduate scholarships found. Check back later!</p>';
    return;
  }
...
function highlightCurrentPage() {
  const currentPage = 'postgraduate.html';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const scholarships = await getPostGraduateScholarships();
